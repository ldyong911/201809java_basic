		 ...
		 *  [7-1] 섯다카드 20장을 포함하는 섯다카드 한 벌(SutdaDeck클래스)을 정의한 것이다. 섯
			...
			void shuffle(){
				for(int i=0; i<cards.length; i++){
					...
					//int temp = cards[i].num;
					//cards[i].num = cards[random].num;
					//cards[random].num = temp;
				}
			}
			...
			[답]
			정의 : 상속관계에서 부모 메서드를 내용을 변경하여 새롭게 재정의 하는것
			필요성 : 부모 클래스의 메서드를 그대로 사용할 수 없는 경우가 있기 때문에
			...
			[답]
			자식클래스에서 Tv() 생성자는 부모클래스의 생성자인 Product()를 호출하는데
			부모클래스에 Product() 생성자가 없기 때문에 오류이므로 Product(){} 를 추가해준다
			
			[7-6] 자손 클래스의 생성자에서 조상 클래스의 생성자를 호출해야하는 이유는 무엇인가?
			[답]
			상속되면 조상 클래스 멤버를 자식 클래스가 사용할 수 있는데
			조상 클래스 멤버의 초기화 작업이 이루어져야 하기 때문이다.
			...
			*[7-16] 다음 중 연산결과가 true가 아닌 것은? (모두 고르시오)
			...
			void add(Product p) {
				if(i >= cart.length){
					...
					System.arraycopy(this.cart, 0, cart, 0, this.cart.length);
					this.cart = cart;
				}
				cart[i] = p;
				i++;
			...
			[답]
			null, Movable 인터페이스를 구현한 클래스 또는 자손의 인스턴스
